# Accelerated Computing Engineer — Progress Dashboard

**Programme:** Accelerated Compute Platform & AI Infrastructure Efficiency Engineer
**Start date:** 18 August 2026
**Target completion:** 30 September 2027
**Target study intensity:** ~15 focused hours/week core + interleaved platform practice
**Primary evidence repository:** accelerated-computing-engineering
**Current status:** ACTIVE

---

# North Star

## Target Professional Identity

**Accelerated Computing & AI Infrastructure Efficiency Engineer**

Able to reason across:

**ALGORITHM → MODEL → COMPILER → KERNEL → ACCELERATOR → MEMORY → INTERCONNECT → CLUSTER → ENERGY → COST**

---

# Overall Progress

| Metric | Current | Target |
|---|---:|---:|
| Programme completion | 0% | 100% |
| Core study hours completed | 0 | ~620 |
| Evidence gates passed | 1 | 16 |
| Major phases completed | 0 | 12 |
| Capstone status | NOT STARTED | PASSED |
| Target completion | — | 30 September 2027 |

---

# Current Focus

**Current Phase:** Phase 1 — Computer Architecture
**Current Course:** Princeton — Computer Architecture
**Current Evidence Gate:** Gate 1 — PASSED
**Current Priority:** Complete the remaining Computer Architecture learning, notes and final diagnostic assessment.

---

# Master Execution Board

| # | Phase / Course | Score | Planned Hours | Completed Hours | Target Window | Evidence Gate | Status |
|---:|---|---:|---:|---:|---|---|---|
| 1 | Princeton — Computer Architecture | 9.5/10 | 40 | 0 | Aug–Sep 2026 | CPU/cache/memory benchmark | ACTIVE |
| 2 | Stanford CS149 — Parallel Computing | 10/10 | 45 | 0 | Sep–Oct 2026 | Parallel scaling benchmark | QUEUED |
| 3 | CU Boulder — HPC Foundations | 9.5/10 | 20 | 0 | Oct 2026 | HPC workload analysis | QUEUED |
| 4 | CU Boulder — Efficient Programming | 9.5/10 | 10 | 0 | Oct 2026 | Performance optimization experiment | QUEUED |
| 5 | CU Boulder — Parallel Computing with MPI | 9.5/10 | 10 | 0 | Oct 2026 | MPI scaling experiment | QUEUED |
| 6 | NVIDIA — CUDA Introduction | 9/10 | 3 | 0 | Oct 2026 | First CUDA kernel | QUEUED |
| 7 | NVIDIA — Modern CUDA C++ | 10/10 | 15 | 0 | Oct–Nov 2026 | CUDA kernel implementation | QUEUED |
| 8 | NVIDIA — CUDA Programming Guide + Labs | 10/10 | 35 | 0 | Nov 2026 | Nontrivial CUDA workload | QUEUED |
| 9 | NVIDIA — Nsight Systems | 10/10 | 10 | 0 | Nov 2026 | Timeline-based diagnosis | QUEUED |
| 10 | NVIDIA — Nsight Compute | 10/10 | 15 | 0 | Nov 2026 | Kernel-level diagnosis | QUEUED |
| 11 | CUDA Memory Optimization Lab | 10/10 | 15 | 0 | Nov–Dec 2026 | Memory optimization result | QUEUED |
| 12 | CUDA Streams / Async / Graphs | 9.5/10 | 15 | 0 | Dec 2026 | Latency-hiding experiment | QUEUED |
| 13 | PyTorch Custom CUDA Extension | 9.5/10 | 12 | 0 | Dec 2026 | Native accelerated PyTorch op | QUEUED |
| 14 | UC Berkeley CS267 — Parallel Computers | 10/10 | 40 | 0 | Dec 2026–Jan 2027 | HPC scalability analysis | QUEUED |
| 15 | AMD AI Academy | 9.5/10 | 10 | 0 | Jan 2027 | ROCm stack understanding | QUEUED |
| 16 | AMD HIP / ROCm Programming | 10/10 | 20 | 0 | Jan 2027 | HIP kernel implementation | QUEUED |
| 17 | CUDA → HIP Port + ROCm Profiling | 10/10 | 20 | 0 | Jan 2027 | NVIDIA vs AMD benchmark | QUEUED |
| 18 | Google TPU Architecture + JAX | 9.5/10 | 20 | 0 | Feb 2027 | JAX accelerator workload | QUEUED |
| 19 | JAX on TPU / Scaling | 9.5/10 | 20 | 0 | Feb 2027 | TPU sharding/scaling experiment | QUEUED |
| 20 | LLVM Compiler Fundamentals | 9.5/10 | 25 | 0 | Feb–Mar 2027 | LLVM IR / compiler artifact | QUEUED |
| 21 | Triton Kernel Programming | 10/10 | 25 | 0 | Mar 2027 | Triton vs CUDA benchmark | QUEUED |
| 22 | PyTorch Compiler / torch.compile | 9.5/10 | 20 | 0 | Mar 2027 | Compiler-generated workload analysis | QUEUED |
| 23 | NVIDIA NCCL + Distributed GPU Systems | 10/10 | 25 | 0 | Mar–Apr 2027 | Multi-GPU scaling analysis | QUEUED |
| 24 | Stanford CS336 — Language Modeling from Scratch | 10/10 | 60 | 0 | Apr–May 2027 | Transformer profiling study | QUEUED |
| 25 | Stanford CME 295 — Selected Modules | 9/10 | 15 | 0 | May 2027 | LLM systems synthesis | QUEUED |
| 26 | Full Stack Deep Learning | 10/10 | 35 | 0 | May–Jun 2027 | Production ML systems artifact | QUEUED |
| 27 | Cross-Accelerator Capstone | 10/10 | 40+ | 0 | Jun–Jul 2027 | Final cross-platform comparison | QUEUED |

---

# Phase Progress

## Phase 1 — Computer Architecture

**Status:** ACTIVE
**Planned hours:** 40
**Completed:** 0
**Progress:** 0%

### Required competence
- [ ] Explain CPU execution pipelines
- [ ] Explain memory hierarchy
- [ ] Explain caches and locality
- [ ] Explain latency vs throughput
- [ ] Understand branch prediction
- [ ] Understand SIMD
- [ ] Understand multicore fundamentals
- [ ] Build cache/memory benchmark
- [ ] Interpret benchmark results

### Evidence Gate 1
**CPU/cache/memory performance experiment**

Status: **PASSED**

---

## Phase 2 — Parallel Computing

**Status:** QUEUED
**Planned hours:** 45
**Completed:** 0

### Evidence Gate 2
**Parallel workload with scaling analysis**

Status: **NOT PASSED**

---

## Phase 3 — High-Performance Computing

**Status:** QUEUED
**Planned hours:** 40
**Completed:** 0

### Evidence Gate 3
**OpenMP + MPI implementation and scaling evidence**

Status: **NOT PASSED**

---

## Phase 4 — CUDA Core

**Status:** QUEUED
**Planned hours:** 53
**Completed:** 0

### Evidence Gate 4
**Correct nontrivial CUDA kernel implementation**

Status: **NOT PASSED**

---

## Phase 5 — GPU Performance Engineering

**Status:** QUEUED
**Planned hours:** 67
**Completed:** 0

### Evidence Gate 5
Complete:

**PROFILE → HYPOTHESIS → OPTIMIZE → RE-PROFILE → QUANTIFY**

Status: **NOT PASSED**

---

## Phase 6 — Advanced HPC

**Status:** QUEUED
**Planned hours:** 40
**Completed:** 0

### Evidence Gate 6
**Compute/communication/scalability analysis**

Status: **NOT PASSED**

---

## Phase 7 — AMD ROCm / HIP

**Status:** QUEUED
**Planned hours:** 50
**Completed:** 0

### Evidence Gate 7
**CUDA workload ported to HIP and benchmarked on AMD**

Status: **NOT PASSED**

---

## Phase 8 — TPU / JAX

**Status:** QUEUED
**Planned hours:** 40
**Completed:** 0

### Evidence Gate 8
**Real workload executed and sharded on TPU**

Status: **NOT PASSED**

---

## Phase 9 — Compiler Systems

**Status:** QUEUED
**Planned hours:** 70
**Completed:** 0

### Evidence Gate 9
**Triton kernel benchmarked against CUDA/PyTorch equivalent**

Status: **NOT PASSED**

---

## Phase 10 — Distributed Accelerators

**Status:** QUEUED
**Planned hours:** 25
**Completed:** 0

### Evidence Gate 10
**Multi-GPU/NCCL scaling experiment**

Status: **NOT PASSED**

---

## Phase 11 — AI Workload Internals

**Status:** QUEUED
**Planned hours:** 110
**Completed:** 0

### Evidence Gate 11
**Transformer workload profiled from model behaviour down to accelerator behaviour**

Status: **NOT PASSED**

---

## Phase 12 — Cross-Accelerator Capstone

**Status:** QUEUED
**Planned hours:** 40+
**Completed:** 0

### Evidence Gate 12

Compare a representative workload across as many of the following as practical:

- CPU baseline
- NVIDIA CUDA
- AMD HIP / ROCm
- Triton
- PyTorch compiler-generated execution
- TPU / JAX

Measure:

- runtime
- throughput
- latency
- memory use
- accelerator utilisation
- scaling
- developer effort
- portability
- power/energy where measurable
- estimated compute cost

Status: **NOT PASSED**

---

# Competency Scoreboard

| Competency | Weight | Current |
|---|---:|---:|
| Computer Architecture | 10% | 0% |
| Parallel Algorithms & HPC | 15% | 0% |
| CUDA Programming | 15% | 0% |
| GPU Profiling & Optimization | 15% | 0% |
| NVIDIA Ecosystem | 10% | 0% |
| AMD ROCm / HIP | 8% | 0% |
| TPU / JAX | 7% | 0% |
| Compiler Systems | 10% | 0% |
| ML Systems | 5% | 0% |
| Cross-Platform Capstone | 5% | 0% |
| **Overall** | **100%** | **0%** |

---

# Monthly Milestones

| Month | Intended State |
|---|---|
| Aug 2026 | Architecture foundation started |
| Sep 2026 | Architecture complete; CS149 active |
| Oct 2026 | Parallel/HPC foundation established; CUDA begins |
| Nov 2026 | CUDA core + profiling active |
| Dec 2026 | GPU performance engineering demonstrated |
| Jan 2027 | Advanced HPC + AMD portability |
| Feb 2027 | TPU/JAX + compiler fundamentals |
| Mar 2027 | Triton/PyTorch compiler + distributed accelerators |
| Apr 2027 | CS336 / LLM internals |
| May 2027 | CS336 + CME295 + production systems |
| Jun 2027 | FSDL complete; capstone active |
| Jul 2027 | Capstone complete; ACE evidence review |

---

# Completion Standard

The programme is not complete because all courses show DONE.

The programme is complete when:

- [ ] All 16 evidence gates pass
- [ ] Core labs are committed
- [ ] Benchmark evidence exists
- [ ] Profiling evidence exists
- [ ] CUDA competence demonstrated
- [ ] AMD portability demonstrated
- [ ] TPU/JAX competence demonstrated
- [ ] Compiler/Triton competence demonstrated
- [ ] Distributed accelerator understanding demonstrated
- [ ] Transformer workload performance analyzed
- [ ] Cross-accelerator capstone completed
- [ ] Final technical synthesis completed

---

# Final Goal

**Target date:** 31 July 2027

At completion I should be able to take an unfamiliar computational workload and:

1. Understand its algorithmic characteristics.
2. Identify relevant hardware constraints.
3. Select an appropriate accelerator architecture.
4. Profile actual execution.
5. Diagnose bottlenecks.
6. Implement or recommend optimizations.
7. Evaluate portability across accelerator stacks.
8. Reason about distributed scaling.
9. Quantify performance improvements.
10. Translate performance into infrastructure and economic impact.

That is the operational definition of an:

# Accelerated Computing & AI Infrastructure Efficiency Engineer
